...
public class ByteArrayBuffer extends OutputStream {
	...
	protected void closeCheck() {
		if(closeFlag) {
			throw new RhilaException("It's already closed.");
		}
	}
	...
	public byte[] toByteArray(boolean readByClear) {
		...
		if(len != allLength) {
			throw new RhilaException("The planned length (" + allLength
	...
	public InputStream getInputStream() {
		return new ByteArrayIOByInputStream(this);
	}
	...
	public OutputStream getOutputStream() {
		return new ByteArrayIOByOutputStream(this);
	}

	// ByteArrayIO用のInputStream.
	private static final class ByteArrayIOByInputStream extends InputStream {
		private final byte[] b1 = new byte[1];
		private ByteArrayBuffer buffer;
		protected ByteArrayIOByInputStream(ByteArrayBuffer buffer) {
			if(buffer == null) {
	...
	/** ByteArrayIO用のOutputStream. **/
	private static final class ByteArrayIOByOutputStream extends OutputStream {
		private final byte[] b1 = new byte[1];
		private ByteArrayBuffer buffer;

		public ByteArrayIOByOutputStream(ByteArrayBuffer buffer) {
			if(buffer == null) {
				throw new NullPointerException();
			}
			this.buffer = buffer;
